...
def main(args):
    ...
    if cfg.DATASET_NAME == 'RSDD':
        ...
        train_dataset_function = get_rsdd_dataset_function(data_dir, 'train')
        MetadataCatalog.get(train_dataset_name).thing_classes = class_labels
...
if __name__ == "__main__":
    ...
    cumem = get_gpu_memory()

    print('#'*20)
    print(current_time)
    print('NGPU :',world_size)
    print('MEMORY :',cumem/1e9)
    print('#'*20)
    ...
    # detectron_dir = os.path.join('detectron2')
    # this_path = os.path.abspath(__file__) # ../train_net.py
    # project_dir = os.path.sep.join(this_path.split(os.path.sep)[:-2])
