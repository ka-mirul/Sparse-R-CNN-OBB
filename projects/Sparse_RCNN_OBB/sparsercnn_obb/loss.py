...
"""
SparseRCNN-OBB model and criterion classes.
"""
import torch
...
class SetCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        src_boxes_ = box_ops.assign_absolute_angle(src_boxes_)
        target_boxes_ = box_ops.assign_absolute_angle(target_boxes_)
    ...
    def _get_src_permutation_idx(self, indices):
        ...
        #batch_idx = tensor([0, 1, 1, 1, 1])
        #src_idx = tensor([7, 0, 5, 6, 7])
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        #out_prob.shape = (num_prop*batch_size, 1)
        #out_bbox.shape = (num_prop*batch_size, 6)
        ...
        out_bbox_ = out_bbox / image_size_out # 0-1 range
        tgt_bbox_ = tgt_bbox / image_size_tgt

        out_bbox_ = box_ops.assign_absolute_angle(out_bbox_)
        tgt_bbox_ = box_ops.assign_absolute_angle(tgt_bbox_)


        cost_bbox = torch.cdist(out_bbox_, tgt_bbox_, p=1) # 0 to 1 range
