...
class FPN(Backbone):
    ...
    def forward(self, x):
        ...
        P5_2x   = F.interpolate(P5, scale_factor=2.0, mode="nearest")
        P5_4x   = F.interpolate(P5, scale_factor=4.0, mode="nearest")
        P5_8x   = F.interpolate(P5, scale_factor=8.0, mode="nearest")
        
 
        P4A     = self.CBAM54(P5_2x)
        P4out   = torch.cat((C4, P4A), dim=1)

        P4      = self.output_convs[1](P4out)
        P4_2x   = F.interpolate(P4, scale_factor=2.0, mode="nearest")
        P4_4x   = F.interpolate(P4, scale_factor=4.0, mode="nearest")

        P3_cat  = torch.cat((P4_2x,P5_4x), dim=1)
        P3A     = self.CBAM43(P3_cat)
        P3out   = torch.cat((C3, P3A), dim=1)
        P3      = self.output_convs[2](P3out)
        P3_2x   = F.interpolate(P3, scale_factor=2.0, mode="nearest")

        P2_cat  = torch.cat((P3_2x, P4_4x, P5_8x), dim=1)
        P2A     = self.CBAM32(P2_cat)
        P2out   = torch.cat((C2, P2A),dim=1)
        P2      = self.output_convs[3](P2out)
